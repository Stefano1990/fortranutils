...
implicit none
  ...
  parameter (ni = 512, nj=140,nk=64,nib=32,njb=140,bpr=16)
...
do row=1,1
do n=16*row-(16-1),row*16
 do j=1,njb
  do i=1,nib
   xp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = x(i,j,n)
   yp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = y(i,j,n)
   up(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = u(i,j,n)/rho(i,j,n)
   vp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = v(i,j,n)/rho(i,j,n)
   wp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = w(i,j,n)/rho(i,j,n)
   sc1p(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = sc1(i,j,n)
   pp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = p(i,j,n)
   rhop(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = rho(i,j,n)
   uup(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = uu(i,j,n)/rho(i,j,n)
   vvp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = vv(i,j,n)/rho(i,j,n)
   wwp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = ww(i,j,n)/rho(i,j,n)
   uvp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = uv(i,j,n)/rho(i,j,n)
   ffp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = ff(i,j,n)
   rrp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = rr(i,j,n)
   sc1varp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb) = sc1var(i,j,n)
   !if(i+(n-(16*row-(16-1)))*nib == 1024 .and. j+(row-1)*njb == 128) then ! i=j=1
   ! write(*,*) 'i=1,j=256;x=',xp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb),'y=',yp(i+(n-(16*row-(16-1)))*nib,j+(row-1)*njb)
   !endif
  enddo
 enddo
 enddo
