...
implicit none
  ...
  double precision :: ev(1:ni+1,1:nj+1,1:maxbl),evp(1:ni*maxbl,1:nj)

open(unit=16, file='flowviz2.dat')
write(16,*) 'VARIABLES = "x" "y" "u" "u-uc" "v" "w" "sc1" "drho" "evisc" '
...
read(m,*)
  do j=1,nj1+1
  do i=1, ni+1
   read(m,*) x(i,j,n), y(i,j,n),u(i,j,n),v(i,j,n),w(i,j,n),sc1(i,j,n),drho(i,j,n),ev(i,j,n)
   enddo
  enddo
 enddo
...
do n=bpr*row-(bpr-1),maxbl
 do j=1,nj1
  do i=1,ni
   ...
   evp(i+(n-(bpr*row-(bpr-1)))*ni,j+(row-1)*nj1) = ev(i,j,n)
  enddo
 enddo
 enddo
enddo
...
write(16,*) xp(i,j), yp(i,j), up(i,j), up(i,j) - uc(i),vp(i,j), wp(i,j), sc1p(i,j),drhop(i,j),evp(i,j)
enddo
enddo
endif
